<?php
/**
 * @copyright  Marko Cupic 2017 <m.cupic@gmx.ch>
 * @author     Marko Cupic
 * @package    Contao Article Class Select Bundle
 * @license    LGPL-3.0+
 * @see	       https://github.com/markocupic/contao-article-class-select-bundle
 *
 */

// Legends
$GLOBALS['TL_LANG']['tl_article']['article_class_select_legend'] = 'Article Layout Einstellungen';


// Fields
$GLOBALS['TL_LANG']['tl_article']['containerClass'] = array('Container-Klasse','Bitte w&auml;hlen Sie die Container-Klasse aus.');
$GLOBALS['TL_LANG']['tl_article']['backgroundClass'] = array('Hintergrund-Klasse','Bitte w&auml;hlen Sie die Hintergrund-Klasse aus.');


// References
$GLOBALS['TL_LANG']['tl_article']['container'] = 'Mittig zentriert';
$GLOBALS['TL_LANG']['tl_article']['container-fluid'] = 'Ganze Breite';
$GLOBALS['TL_LANG']['tl_article']['container-fluid p-0 m-0'] = 'Ganze Breite ohne padding und margin';
